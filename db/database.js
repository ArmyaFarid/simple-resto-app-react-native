...
export function saveMenuItems(menuItems) {
  db.transaction((tx) => {
    // Extract values from each menu item to construct the SQL statement
    const values = menuItems.map((item) => `(${item.id}, '${item.title}', ${item.price}, '${item.category}')`).join(',');


    // Construct the SQL statement to insert multiple rows at once
    const sqlStatement = `INSERT INTO menuitems (id, title, price, category) VALUES ${values}`;
...
//   return new Promise((resolve, reject) => {
//     const allRows =  db.getAllAsync('SELECT * FROM menuitems WHERE category = ? AND title LIKE ?' , query, activeCategories);
...
export async function filterByQueryAndCategories(query, activeCategories) {
  return new Promise((resolve, reject) => {
    db.transaction((tx) => {
      // Construct the SQL statement with conditions for both query and categories
      const sqlStatement = `
        SELECT *
        FROM menuitems
      ...
      // Concatenate the "%" wildcard to match substrings in the query
      const searchString = `%${query}%`;

      console.log(`${activeCategories.map(() => '?').join(',')}`)

      // Execute the SQL statement with parameters
      tx.executeSql(sqlStatement, [searchString, ...activeCategories], (tx, result) => {
        const filteredMenuItems = [];

        // Iterate over the result set and extract the filtered menu items
        for (let i = 0; i < result.rows.length; i++) {
          const menuItem = result.rows.item(i);
        ...
        // Resolve the promise with the filtered menu items
